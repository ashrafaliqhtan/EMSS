...
def conflict_dashboard(request):
    ...
    lecturer_conflicts = Lecturer.objects.annotate(
        conflict_count=models.Count(
            'examhallassignment_first_supervisor',
            filter=~Q(examhallassignment_first_supervisor__resolved=True) |
                  ~Q(examhallassignment_second_supervisor__resolved=True)
        )
    ...
    hall_conflicts = ExamHallAssignment.objects.filter(
        ...
        conflict_count=models.Count('id', filter=Q(conflict__resolved=False))
...
def dashboard(request):
    ...
    return render(request, 'invigilation/dashboard.html', stats)
...
def reports_backend(request):
    """
    Backend reports view for admin users.
    Aggregates detailed data for administrative reporting and prepares data for charts.
    """
    # Aggregate exam counts by exam_date (assuming Exam model has an exam_date field)
    exam_report = Exam.objects.values('exam_date').annotate(count=Count('id')).order_by('exam_date')
    # Prepare data for chart: lists of dates and counts
    exam_dates = [item['exam_date'].strftime("%Y-%m-%d") for item in exam_report if item['exam_date']]
    exam_counts = [item['count'] for item in exam_report]
    
    # Aggregate lecturer counts grouped by invigilation type
    lecturer_report = Lecturer.objects.values('invigilation_type').annotate(count=Count('id'))
